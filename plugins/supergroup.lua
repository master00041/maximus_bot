...
local function check_member_super(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = 'سوپر گروه با موفقیت ادد شد!'
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = 'سوپر گروه از لیست گروه ها پاک شد'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if not is_owner(msg) then
    return "فقط ادمین کونی
  end
  local group_spam_lock = data[tostring(target)]['settings']['lock_spam']
  if group_spam_lock == 'yes' then
    return 'اسپم از قبل قفل بود جقی'
  else
...
local function unlock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'no' then
    return 'اسپم از قبل باز بود خل من'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'no'
    save_data(_config.moderation.data, data)
    return 'اسپم در سوپر گروه ازاد است'
  end
end
...
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'yes' then
    return 'از قبل قفل بود گل من'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'عربی کص بگن پاک میکنم'
  end
end
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return 'کضخل از قبل باز بود'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل عربی باز شد میتونین کص بگین'
  end
end
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return 'کصخل از قبل قفل بود'
  else
    data[tostring(target)]['settings']['lock_member'] = 'yes'
    save_data(_config.moderation.data, data)
  end
  return 'ادد کردن کصخل قفل شد'
end
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return 'قفل ممبر باز شد میتونی ادد کنی'
  end
end
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'yes' then
    return 'قفله کصخل جقی'
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'ازین به بعد استیکر بدین پاک میکنم'
  end
end
...
local function unlock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'no' then
    return 'قفل نیس خل من'
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل استیکر باز شد!کص بگین'
  end
end
...
local function lock_group_fwd(msg, data, target)
  ...
  if group_fwd_lock == 'yes' then
    return reply_msg(msg.id,"از قبل قفله جقی", ok_cb, false)
  else
    data[tostring(target)]['settings']['lock_fwd'] = 'yes'
    save_data(_config.moderation.data, data)
    return reply_msg(msg.id,"خایه داری فووروارد کن حالا", ok_cb, false)
  end
end
...
local function unlock_group_fwd(msg, data, target)
  ...
  if group_fwd_lock == 'no' then
    return reply_msg(msg.id,"از قبل باز بود کصخل", ok_cb, false)
  else
    data[tostring(target)]['settings']['lock_fwd'] = 'no'
    save_data(_config.moderation.data, data)
    return reply_msg(msg.id,"قفل فووروارد باز شد", ok_cb, false)
  end
end
...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'قفل' and is_momod(msg) then
			...
			if matches[2] == 'تنظیمات سخت گیرانه' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked enabled strict settings")
				return enable_strict_rules(msg, data, target)
			end
			if matches[2] == 'links' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked forward message")
				return group_fwd_lock(msg, data, target)
		end

		if matches[1] == 'باز کن' and is_momod(msg) then
			local target = msg.to.id
			if matches[2] == 'لینک' then
		...
		if matches[1] == 'تنظیم حساسیت' then
			...
			if tonumber(matches[2]) < 5 or tonumber(matches[2]) > 20 then
				return "شماره اشتباه حساسیت میتواند بین 5تا20 باشد"
			end
			local flood_max = matches[2]
			data[tostring(msg.to.id)]['settings']['flood_msg_max'] = flood_max
			save_data(_config.moderation.data, data)
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] set flood to ["..matches[2].."]")
			return 'حساسیت اسپم تنظیم شد به: '..matches[2]
		end
		...
		if matches[1] == 'خفه کن' and is_owner(msg) then
			local chat_id = msg.to.id
			...
			if matches[2] == 'همرو' then
			local msg_type = 'All'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "همه "..msg_type.."  خفه شدند"
				else
					return "همه "..msg_type.." از قبل خفه بودند"
				end
			end
		end
		if matches[1] == 'خفه نکن' and is_momod(msg) then
			local chat_id = msg.to.id
			if matches[2] == 'وویس' then
			local msg_type = 'Audio'
		...
		if matches[1] == 'راهنما' and not is_owner(msg) then
			text = "برای راهنما متن راهنما را به پی وی بات بفرستید"
			reply_msg(msg.id, text, ok_cb, false)
